...
void IMU::send_serial::set_filter_parameters()
{
	...
	{
		std::vector<uint8_t> x(float_to_raw(-0.25f)), y(float_to_raw(0.0f)), z(float_to_raw(0.25f));
	...
	{
		std::vector<uint8_t> x(float_to_raw(-0.51f)), y(float_to_raw(-0.05f)), z(float_to_raw(-0.04f));
