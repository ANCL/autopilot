...
void IMU::send_serial::set_filter_parameters()
{
	...
	{
		std::vector<uint8_t> x(float_to_raw(0.052f)), y(float_to_raw(0.0f)), z(float_to_raw(-0.0213f));
	...
	{
		std::vector<uint8_t> x(float_to_raw(-0.72f)), y(float_to_raw(0.0f)), z(float_to_raw(-0.373f));
